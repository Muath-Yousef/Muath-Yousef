# Mu'ath Yousef

Cybersecurity Engineer focused on practical SOC engineering, security automation, and evidence-driven security operations.

I am building **SOCRoot**, an evolving security engineering initiative that grew from **Project Synapse**. The work explores how SIEM, SOAR, case management, evidence capture, observability, and AI-assisted analysis can be combined into safer and more accessible security operations.

## Current focus

- SOC, SIEM, and SOAR engineering
- Detection and incident-response workflows
- Human-in-the-loop security automation
- Evidence capture and audit-friendly reporting
- Security architecture for small and medium-sized organizations
- Safe integration of AI into security operations

## SOCRoot repository map

| Layer | Repository | Purpose |
|---|---|---|
| Public architecture | [project-synapse](https://github.com/Muath-Yousef/project-synapse) | Architecture, engineering principles, maturity, and roadmap |
| Product website | [kyriesoc](https://github.com/Muath-Yousef/kyriesoc) | Public SOCRoot website and product communication |
| Engineering portfolio | [portfolio-site](https://github.com/Muath-Yousef/portfolio-site) | Selected work, delivery principles, and project maturity |
| Assessment evidence | [Reports](https://github.com/Muath-Yousef/Reports) | Authorized, lab-scoped security assessment artifacts |

The canonical control plane and SOC runtime are private while their Git histories undergo security review. For a status-aware view of the account—including active, archived, private, and safety-review work—see the [repository guide](REPOSITORY_GUIDE.md).

## Selected evidence

- [DVTA Security Assessment](https://muath-yousef.github.io/Reports/) — authorized grey-box assessment of DVTA v2.0 in an isolated lab, documenting 12 confirmed findings with evidence and mitigations
- [SOCRoot architecture](https://github.com/Muath-Yousef/project-synapse) — public system boundaries, maturity, threat model, and roadmap

## Technologies

Wazuh · TheHive · Shuffle · Python · FastAPI · Docker · PostgreSQL · Redis · OpenSearch · Next.js · Cloudflare WAF · ChromaDB · Prometheus · Grafana

## Engineering principles

- Sensitive remediation stays human-controlled
- SOAR dry-run is the default
- CDN and RFC1918 ranges are never automatically blocked
- DNS events remain notification-only
- Raw client data is not sent to external AI providers
- Production claims require documented tests, rollback paths, and operational evidence

## Connect

[SOCRoot](https://socroot.com) · [LinkedIn](https://www.linkedin.com/in/muath-ysf) · [GitHub](https://github.com/Muath-Yousef)
